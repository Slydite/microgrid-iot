...
class MeasurementOneConsumer(WebsocketConsumer):
    def connect(self):
        self.accept()
    ...
    def send_latest_measurement(self, event):
        latest_measurement = MeasurementsOne.objects.latest('time')
        self.send(text_data=json.dumps({
            'voltage': str(latest_measurement.voltage),
            'time': str(latest_measurement.time),
            'sensor_id': str(latest_measurement.sensor_id),
        }))
